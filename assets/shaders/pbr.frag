...
uniform float time; // for ripple
uniform float ao = 1.0;
uniform bool useWet = false;
...
void main() {
    ...
    if (useWet) {
        ...
        vec3 N = normalize(TBNMat * normalize(rippleNormal));
    } else {
        vec3 normalMapSample = texture(normalMap, texCoord).rgb * 2.0 - 1.0;
        vec3 N = normalize(TBNMat * normalMapSample);
    }
